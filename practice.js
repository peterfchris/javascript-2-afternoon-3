...
function each(nameArr, cb) {
  for (var index = 0; index <= nameArr.length - 1; index++) {
    cb(nameArr[index], index)
  }
}
// Do not edit the code below.
...
function getUserById(users, id, cb) {
  for (var index = 0; index <= users.length - 1; index++) {    
    // below user is getting from users array user based off of index
    var user = users[index]
    // below accessing id property of user obj
    if (user.id === id) {
        cb(users[index])
      }
  }
